...
function effectiveServiceStatus(r:Service){
  if(r.status==='cancelado') return 'cancelado';
  if(r.end_date){
    const end=String(r.end_date).slice(0,10);
    if(end < today()) return 'finalizado';
  }
  return r.status==='finalizado' ? 'finalizado' : 'activo';
}

export function ServiceManager({embedded=false}:{embedded?:boolean}={}){
  ...
  return <div className="page-stack">{!embedded&&<SectionTitle title="Servicios" subtitle="Servicios puntuales o recurrentes. Los mensualizados calculan automáticamente vencimiento contractual y valor total." action={<button className="primary-button" onClick={openNew}>+ Nuevo servicio</button>}/>}
    {embedded&&<div style={{display:'flex',justifyContent:'flex-end'}}><button className="primary-button" onClick={openNew}>+ Nuevo servicio</button></div>}{error?<ErrorBox message={error} onRetry={load}/>:loading?<Loading/>:<Card><div className="table-toolbar"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar servicios…"/><span className="record-count">{filtered.length} registros</span></div>{filtered.length===0?<Empty text="Todavía no hay servicios."/>:<div className="table-wrap"><table><thead><tr><th>Servicio</th><th>Cliente</th><th>Modalidad</th><th>Monto período</th><th>Duración</th><th>Valor total</th><th>Desde</th><th>Hasta</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{filtered.map(r=>{const effectiveStatus=effectiveServiceStatus(r);return <tr key={r.id} className="clickable-row" onClick={()=>setSelected(r.id)}><td>{r.name}</td><td>{clientMap[r.client_id]||'—'}</td><td>{r.service_type}</td><td><b>{money(r.billing_amount)}</b></td><td>{r.duration_months?`${r.duration_months} meses`:'—'}</td><td><b>{money(r.contract_amount)}</b></td><td>{r.start_date?dateAR(r.start_date):'—'}</td><td>{r.end_date?dateAR(r.end_date):'—'}</td><td><Status tone={effectiveStatus==='activo'?'green':effectiveStatus==='cancelado'?'red':'blue'}>{effectiveStatus}</Status></td><td><div className="row-actions"><button className="mini-button" onClick={e=>{e.stopPropagation();openEdit(r)}}>Editar</button><button className="mini-button danger-text" onClick={e=>{e.stopPropagation();remove(r)}}>Eliminar</button></div></td></tr>})}</tbody></table></div>}</Card>}
