...
function Gantt({rows}:{rows:any[]}){
 const dated=rows.filter(r=>r.start_date&&r.end_date);
 if(dated.length===0)return <Card><Empty text="Cargá fechas de inicio y fin para visualizar el cronograma."/></Card>;

 const starts=dated.map(r=>new Date(`${String(r.start_date).slice(0,10)}T12:00:00`).getTime());
 const ends=dated.map(r=>new Date(`${String(r.end_date).slice(0,10)}T12:00:00`).getTime());
 const min=new Date(Math.min(...starts));
 const max=new Date(Math.max(...ends));
 const totalDays=Math.max(1,Math.round((max.getTime()-min.getTime())/86400000)+1);
 const ticks=Array.from({length:Math.min(totalDays,14)},(_,i)=>new Date(min.getTime()+Math.round(i*(totalDays-1)/Math.max(1,Math.min(totalDays,14)-1))*86400000));

 return <Card>
  <SectionTitle title="Cronograma de ejecución" subtitle={`${fmtDate(iso(min))} → ${fmtDate(iso(max))}`}/>
  <div className="gantt">
   <div className="gantt-header"><div></div><div className="gantt-scale">{ticks.map((d,i)=><span key={i} style={{left:`${i/Math.max(1,ticks.length-1)*100}%`}}>{d.toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit'})}</span>)}</div></div>
   {dated.map(r=>{
    const a=new Date(`${String(r.start_date).slice(0,10)}T12:00:00`).getTime();
    const b=new Date(`${String(r.end_date).slice(0,10)}T12:00:00`).getTime();
    const left=(a-min.getTime())/Math.max(86400000,max.getTime()-min.getTime()+86400000)*100;
    const width=Math.max(2,(b-a+86400000)/Math.max(86400000,max.getTime()-min.getTime()+86400000)*100);
    return <div className="gantt-row" key={r.id}>
     <div><b>{r.title}</b><small>{r.work_name}{r.responsible?` · ${r.responsible}`:''}</small></div>
     <div className="gantt-track"><div className={`gantt-bar ${r.status} ${r.is_overdue?'overdue':''}`} style={{left:`${left}%`,width:`${width}%`}}><i style={{width:`${Math.min(100,Number(r.progress_percent||0))}%`}}/></div></div>
    </div>
   })}
  </div>
 </Card>
