      ...
      <div className="table-wrap works-board-table-wrap">
       <table className="works-board-table services-board-table">
        <thead><tr>
          ...
          <th>Períodos</th><th>Facturado</th><th>Cobrado</th><th>Pendiente</th><th>Próximo período</th><th>Riesgo</th><th>Acciones</th>
        </tr></thead>
        <tbody>{rows.map((r:any)=>{
          const progress=Math.max(0,Math.min(100,Number(r.billing_progress_percent||0)));
          return <tr key={r.id} className={r.effective_status!=='activo'?'finished-row':''}>
            <td><b>{r.name}</b>
