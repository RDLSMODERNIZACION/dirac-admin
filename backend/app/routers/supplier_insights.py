...
def ensure_schema(cur):
    ...
    # Clasificación inicial para proveedores existentes sin grupo.
    cur.execute(sql.SQL("""
        UPDATE {}.suppliers
        SET supplier_group = CASE
            WHEN LOWER(name) LIKE ANY(ARRAY[
                '%facebook%','%meta%','%instagram%','%marketing%','%publicidad%',
                '%imprenta%','%diseño%','%freeda%'
            ]) THEN 'marketing'
            WHEN LOWER(name) LIKE ANY(ARRAY[
                '%seguro%','%microtrack%','%nippon%','%taller%','%combustible%',
                '%lubricentro%','%neumatic%','%repuesto%','%vehicul%','%camion%',
                '%tracker%','%gps%'
            ]) THEN 'flota_vehicular'
            ELSE 'contratistas'
        END
...
def load_suppliers(cur):
    cur.execute(sql.SQL("""
        WITH payable_calc AS (
            ...
            WHERE p.supplier_id IS NOT NULL
              AND LOWER(COALESCE(p.status,'')) <> 'anulado'
        ...
        this_month AS (
            SELECT
                p.supplier_id,
                COALESCE(SUM(fm.amount),0) AS paid_this_month
        ...
        SELECT
            s.id,s.name,s.tax_id,s.type,s.contact_name,s.email,s.phone,s.address,
            s.notes,s.is_active,s.supplier_group,
        ...
        ORDER BY
            CASE s.supplier_group
                WHEN 'flota_vehicular' THEN 1
    ...
    for r in rows:
        ...
        if overdue > 0 and (max_days >= 30 or pending > 0):
            r["risk_level"] = "alto"
            r["risk_reason"] = f"{max_days} días de atraso" if max_days else "saldo vencido"
        elif pending > 0:
            r["risk_level"] = "medio"
            r["risk_reason"] = "saldo pendiente"
        else:
            r["risk_level"] = "bajo"
            r["risk_reason"] = "sin deuda pendiente"
    return rows
...
def supplier_insights():
    ...
    return {
        "summary": {
            ...
            "flota_vehicular": by_group["flota_vehicular"],
            "marketing": by_group["marketing"],
            "contratistas": by_group["contratistas"],
...
def supplier_detail(supplier_id: UUID):
    with db_cursor() as cur:
        ensure_schema(cur)
        suppliers = load_suppliers(cur)
        supplier = next((r for r in suppliers if r["id"]==supplier_id), None)
        if not supplier:
            raise HTTPException(404, "Proveedor no encontrado")

        cur.execute(sql.SQL("""
            SELECT
                p.id,p.description,p.document_number,p.issue_date,p.due_date,p.amount,p.status,
                COALESCE(SUM(fm.amount) FILTER (WHERE fm.type='egreso'),0) AS paid,
            ...
            WHERE p.supplier_id=%s
              AND LOWER(COALESCE(p.status,''))<>'anulado'
            GROUP BY p.id
            ORDER BY p.due_date DESC NULLS LAST
        """).format(S,S), [supplier_id])
        payables = cur.fetchall()

        cur.execute(sql.SQL("""
            SELECT
                fm.id,fm.movement_date,fm.amount,fm.description,fm.category,
                p.document_number,p.description AS payable_description
            FROM {}.financial_movements fm
            JOIN {}.payables p ON p.id=fm.payable_id
            WHERE p.supplier_id=%s AND fm.type='egreso'
            ORDER BY fm.movement_date DESC NULLS LAST
        """).format(S,S), [supplier_id])
        payments = cur.fetchall()

        cur.execute(sql.SQL("""
            SELECT id,document_type,title,document_date,url,notes,created_at
...
def create_supplier(body: SupplierPayload):
    ...
    with db_cursor() as cur:
        ...
        cur.execute(sql.SQL("""
            ...
            (name,supplier_group,tax_id,type,contact_name,email,phone,address,notes,is_active)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            RETURNING *
        """).format(S), [
            body.name.strip(),body.supplier_group,body.tax_id,body.type,body.contact_name,
            body.email,body.phone,body.address,body.notes,body.is_active
        ])
        return cur.fetchone()
    ...
    with db_cursor() as cur:
        ...
        cur.execute(sql.SQL("""
            UPDATE {}.suppliers SET
                name=%s,supplier_group=%s,tax_id=%s,type=%s,contact_name=%s,email=%s,
                phone=%s,address=%s,notes=%s,is_active=%s
            WHERE id=%s
            RETURNING *
        """).format(S), [
            body.name.strip(),body.supplier_group,body.tax_id,body.type,body.contact_name,
            body.email,body.phone,body.address,body.notes,body.is_active,supplier_id
        ])
        row = cur.fetchone()
        if not row:
            raise HTTPException(404, "Proveedor no encontrado")
...
def delete_supplier(supplier_id: UUID):
    with db_cursor() as cur:
        ...
        cur.execute(sql.SQL("""
            SELECT
                EXISTS(SELECT 1 FROM {}.payables WHERE supplier_id=%s) AS has_payables,
        ...
        """).format(S,S,S), [supplier_id,supplier_id,supplier_id])
        links = cur.fetchone()
        if links and any(links.values()):
            raise HTTPException(
                400,
                "No se puede eliminar porque tiene movimientos asociados. Podés marcarlo como inactivo."
            )

        cur.execute(
            sql.SQL("DELETE FROM {}.suppliers WHERE id=%s RETURNING id").format(S),
            [supplier_id],
        )
        if not cur.fetchone():
            raise HTTPException(404, "Proveedor no encontrado")
...
def add_document(supplier_id: UUID, body: DocumentPayload):
    if not body.title.strip():
        raise HTTPException(400, "El documento necesita un título")
    with db_cursor() as cur:
        ensure_schema(cur)
        ...
        cur.execute(sql.SQL("""
            ...
            (supplier_id,document_type,title,document_date,url,notes)
            VALUES (%s,%s,%s,%s,%s,%s)
            RETURNING *
        """).format(S), [
            supplier_id,body.document_type,body.title.strip(),body.document_date,body.url,body.notes
        ])
        return cur.fetchone()
...
def delete_document(document_id: UUID):
    with db_cursor() as cur:
        ensure_schema(cur)
        cur.execute(
            sql.SQL("DELETE FROM {}.supplier_documents WHERE id=%s RETURNING id").format(S),
            [document_id],
        )
        if not cur.fetchone():
